*poetest by Julian Sagebiel 19.09.2016
*version 0.3
*Changes to v0.2: meancalculations are deleted as they cause error when using currentdata 

capture program drop poetestc
capture mata: mata drop poe()
capture mata: mata drop krdraw()

program poetestc, rclass
version 11
syntax namelist,  [model1(string) model2(string) price(name) currentdata repl(integer 1000) seed(integer 5426)]
...
mat b=e(b)
mat v=e(V)
local attrnames: colnames b
local namenum: list posof "`price'" in attrnames
if `namenum'== 0 {
...
mat `wtp`i''=`wtp`i''[.,"`namelist'"]
}
mat wtp=`wtp1'[.,"`namelist'"],`wtp2'[.,"`namelist'"]

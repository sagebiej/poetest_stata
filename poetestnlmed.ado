*poetest for model with price log normal to compare WTP based on median price exp(price) by Julian Sagebiel 17.12.2017
*version 0.4

capture program drop poetestnlmed
capture mata: mata drop poe()
capture mata: mata drop krdraw()

program poetestnlmed, rclass
version 11
...
scalar mean`i' =_b[Mean:`namelist']/ exp(_b[`price'])
